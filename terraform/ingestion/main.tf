...
resource "aws_s3_bucket" "vectors" {
  bucket = "mlbdraftoracle-vectors-${data.aws_caller_identity.current.account_id}"
  
  tags = {
    Project = "mlbdraftoracle"
    Part    = "3"
  }
}
...
resource "aws_iam_role" "lambda_role" {
  name = "mlbdraftoracle-ingest-lambda-role"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name = "mlbdraftoracle-ingest-lambda-policy"
...
resource "aws_lambda_function" "ingest" {
  function_name = "mlbdraftoracle-ingest"
  role          = aws_iam_role.lambda_role.arn
  
  # Note: The deployment package will be created by the guide instructions
  filename         = "${path.module}/../../ingest/lambda_function.zip"
  source_code_hash = fileexists("${path.module}/../../ingest/lambda_function.zip") ? filebase64sha256("${path.module}/../../ingest/lambda_function.zip") : null
...
resource "aws_cloudwatch_log_group" "lambda_logs" {
  name              = "/aws/lambda/mlbdraftoracle-ingest"
  retention_in_days = 7
...
resource "aws_api_gateway_rest_api" "api" {
  name        = "mlbdraftoracle-api"
  description = "MLB Draft Oracle API"
...
resource "aws_api_gateway_usage_plan" "plan" {
  name = "mlbdraftoracle-usage-plan"
