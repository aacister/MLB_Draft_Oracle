...
def get_service_url():
    ...
    try:
        ...
        result = subprocess.run([
            ...
            "--query", "ServiceSummaryList[?ServiceName=='mlbdraftoracle-researcher'].ServiceArn",
            "--output", "json"
        ], capture_output=True, text=True, check=True)
        
        service_arns = json.loads(result.stdout)
        if not service_arns:
            print("❌ App Runner service 'mlbdraftoracle-researcher' not found.")
...
def main():
    parser = argparse.ArgumentParser(
        description="Test the MLB Draft Oracle Researcher service",
...
Examples:
  ...
  uv run test_research.py "Microsoft cloud revenue growth"
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Topic to research (optional - agent will pick trending topic if not provided)"
    )
